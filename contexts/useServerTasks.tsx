...
import { createTaskForServerRelation, editTaskFromServerRelation, getServerTasksByRelationId, removeTaskFromServerRelation } from "@/service/database";
const useServerTasks = () => {
  ...
  const addTaskToDb = async (newTask : Omit<TaskType, 'id'>) => {
    ...
    const response = await createTaskForServerRelation(relation.current.id, newTask);
  ...
  const removeTaskFromDb = async (ids: string|string[]) => {
    if(Array.isArray(ids)){
      const promises = ids.map(id => removeTaskFromServerRelation(relation.current?.id as string, id));
      const response =  await Promise.all(promises);
      setTasks(tasks.filter(t => !response.filter(i => i !== null).map(i => i!.id).includes(t.id)));
      return response;
    }
    const data = await removeTaskFromServerRelation(relation.current?.id as string, ids);
  ...
  const toggleTaskInDb = async (id: string) => {
    ...
    if(user?.id && togglableTask){
      const response = isToggled(togglableTask)
        ? await editTaskFromServerRelation(togglableTask.task_relations_id, { id, completed_at:null, completed_by: null})
        : await editTaskFromServerRelation(togglableTask.task_relations_id, { id, completed_at: getSQLiteTimestamp(), completed_by: user?.id});

      console.log(response);
      setTasks(prevTasks => [...prevTasks.map(t => t.id ===id ? response: t)]);
    }
  };
