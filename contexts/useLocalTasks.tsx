  ...
  const toggleTaskInDb = async (id: string) => {
    ...
    if(user?.id && togglableTask){
      isToggled(togglableTask)
        ? await toggleTask({id:togglableTask.id, completed_at:null, completed_by: null})
        : await toggleTask({id:togglableTask.id, completed_at: getSQLiteTimestamp(), completed_by: user?.id});
    }
  };
